...
public class ArrayList1 {
	public static void main(String args[]){
		...
		/* suppression des elements d'une position donn�e */
		v.remove(3);
		v.remove(5);
		v.remove(5);
		System.out.println("En C : contenu v = "+v );
		
		/* ajout d'element a une position donn�es */
		v.add(2, new Integer(100));  
		v.add(2, new Integer(200));
		System.out.println("En D : contenu v = "+v );
		
		/* modification d'element d'une position donn�es */
		v.set(2, new Integer(1000)); // modification element de range 2
		v.set(5, new Integer(2000));
		System.out.println("En E : contenu v = "+v );		
		/* il y a la methode  ensureCapacity(minCapacity); pour les vecteurs demande d'allouer au vecteur une capacite au moins egals a capacit� minima
		 * il y a aussi trimToSize() demande de ramener la capacit� du vecteur � sa taille actuelle en lib�rant les espace memoires non utilis� c'est utile quand on sait que la taille ne change pas   
		 */
